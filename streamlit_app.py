...
def current_setup_dict() -> dict:
    keys = [
        ...
        "APPLY_PART_LOAD_COP_TO_CORE", "APPLY_LATENT_LOAD_TO_CORE", "APPLY_HX_AIR_PRESSURE_TO_FAN", "APPLY_HX_WATER_PRESSURE_TO_PUMP", "APPLY_HX_UA_TO_CAPACITY", "APPLY_NATIVE_ZONE_LOADS", "APPLY_DUAL_SETPOINT_MIXED_MODE_TO_CORE", "MIXED_MODE_MIN_LOAD_FRACTION", "APPLY_THERMAL_MASS_LAG_TO_CORE",
...
with tabs[1]:
    ...
    if quick[0].button("Enable all main terms"):
        for k in ["USE_ENVELOPE", "USE_WALLS", "USE_ROOF", "USE_WINDOWS", "USE_SOLAR", "USE_INFILTRATION", "USE_INTERNAL_GAINS", "USE_PEOPLE_GAINS", "USE_LIGHTING_GAINS", "USE_EQUIPMENT_GAINS", "USE_HVAC_FANS", "USE_HVAC_PUMPS", "USE_HVAC_AUXILIARY", "USE_COOLING", "USE_HEATING", "USE_DEGRADATION", "USE_CARBON", "USE_MAINTENANCE_COST", "APPLY_PART_LOAD_COP_TO_CORE", "APPLY_LATENT_LOAD_TO_CORE", "APPLY_HX_AIR_PRESSURE_TO_FAN", "APPLY_HX_WATER_PRESSURE_TO_PUMP", "APPLY_HX_UA_TO_CAPACITY", "APPLY_NATIVE_ZONE_LOADS", "APPLY_DUAL_SETPOINT_MIXED_MODE_TO_CORE", "APPLY_THERMAL_MASS_LAG_TO_CORE"]:
    ...
    if st.button("Reset coupled modules OFF / reproduce original core"):
        for k in ["APPLY_PART_LOAD_COP_TO_CORE", "APPLY_LATENT_LOAD_TO_CORE", "APPLY_HX_AIR_PRESSURE_TO_FAN", "APPLY_HX_WATER_PRESSURE_TO_PUMP", "APPLY_HX_UA_TO_CAPACITY", "APPLY_NATIVE_ZONE_LOADS", "APPLY_DUAL_SETPOINT_MIXED_MODE_TO_CORE", "APPLY_THERMAL_MASS_LAG_TO_CORE"]:
            st.session_state[k] = False
...
def _disable_coupled_modules(new_cfg: HVACConfig) -> HVACConfig:
    for attr in [
        ...
        "APPLY_HX_WATER_PRESSURE_TO_PUMP", "APPLY_HX_UA_TO_CAPACITY", "APPLY_NATIVE_ZONE_LOADS", "APPLY_DUAL_SETPOINT_MIXED_MODE_TO_CORE",
    ]:
        if hasattr(new_cfg, attr):
